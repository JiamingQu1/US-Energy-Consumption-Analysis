...
ggplot(allu_data, aes(axis1 = MONEYPY_cat, axis2 = COOLCNTL_cat, axis3 = TYPEHUQ)) +
  ...
  theme_minimal()
```
We observe that there is still a significant proportion of people in every 

Apart from habits on controlling cooling devices (such as AC), the use of LED lights.
```{r}
# Create the alluvial plot
ggplot(allu_data, aes(axis1 = MONEYPY_cat, axis2 = LGTINLED, axis3 = TYPEHUQ)) +
  ...
  scale_fill_brewer(palette = "Set2") +
  labs(
    title = "Alluvial Plot: Income, LED Use Frequency and cooling",
    x = "Variables",
    y = "Frequency"
  ) +
  theme_minimal()
